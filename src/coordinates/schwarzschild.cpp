  ...
  : Coordinates(pmb, pin, flag)
{
  ...
  for (int i = il-ng; i <= iu+ng; ++i) {
    ...
    metric_cell_i1_(i) = _sqr(alpha_c);
  }
  ...
  if (coarse_flag == false) {
    ...
    for (int i = il-ng; i <= iu+ng; ++i) {
      ...
      coord_src_i3_(i) = 2.0*m - 3.0/4.0 * (r_m + r_p) * (_sqr(r_m) + _sqr(r_p))
          / (_sqr(r_m) + r_m * r_p + _sqr(r_p));
      coord_src_i4_(i) = 3.0/2.0 * (r_m + r_p) / (_sqr(r_m) + r_m * r_p + _sqr(r_p));

      // Metric coefficients
      metric_face1_i1_(i) = _sqr(alpha_m);
      if (i == (iu+ng)) {
        metric_face1_i1_(i+1) = _sqr(alpha_p);
      }
      metric_face2_i1_(i) = _sqr(alpha_c);
      metric_face3_i1_(i) = _sqr(alpha_c);
