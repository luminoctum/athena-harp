...
#pragma simd
      for (int i=pmy_block->is; i<=pmy_block->ie; ++i) {
        ...
        Real m_ii = prim(IDN,k,j,i)*(_sqr(prim(IM2,k,j,i)) + _sqr(prim(IM3,k,j,i)));
        ...
        Real m_pp = prim(IDN,k,j,i)*_sqr(prim(IM3,k,j,i));
        if (NON_BAROTROPIC_EOS) {
          m_pp += prim(IEN,k,j,i);
        } else {
        ...
        if (MAGNETIC_FIELDS_ENABLED) {
           m_pp += 0.5*( _sqr(bcc(IB1,k,j,i)) + _sqr(bcc(IB2,k,j,i)) - _sqr(bcc(IB3,k,j,i)) );
        }
