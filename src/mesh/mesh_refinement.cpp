...
MeshRefinement::MeshRefinement(MeshBlock *pmb, ParameterInput *pin)
{
  ...
  if (COORDINATE_SYSTEM == "cartesian") {
    ...
    pcoarsec = new SphericalLatlon(pmb, pin, true);
...
#pragma unroll
          ...
          Real Sdx1=_sqr(pco->dx1f(fi)+pco->dx1f(fi+1));
          Real Sdx2=_sqr(pco->GetEdge2Length(fk+1,fj,fi+1)+
                        pco->GetEdge2Length(fk+1,fj+1,fi+1));
          Real Sdx3=_sqr(pco->GetEdge3Length(fk,fj+1,fi+1)+
