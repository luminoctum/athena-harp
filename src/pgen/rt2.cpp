...
#pragma simd
        for (int i=is; i<=ie; ++i) {
          Real den = prim(IPR,k,js+j-1,i)*2.0E-3/(Globals::Rgas*prim(IT,k,js+j-1,i));
          prim(IPR,k,js-j,i) = prim(IPR,k,js+j-1,i) 
             - den*grav_acc*(2*j-1)*pco->dx2f(j)/gm1;
        }
      } else {
#pragma simd
        for (int i=is; i<=ie; ++i) {
  ...
  for (int k = ks; k <= ke; ++k)
    for (int j = 1; j <= NGHOST; ++j)
      for (int i = is; i <= ie; ++i) {
        ...
        Real den = prim(IPR,k,js+j-1,i)*2.0E-3/(Globals::Rgas*prim(IT,k,js+j-1,i));
        prim(IPR,k,js-j,i) = prim(IPR,k,js+j-1,i) - den*grav_acc*(2*j-1)*pco->dx2f(j)/gm1;
...
#pragma simd
        for (int i=is; i<=ie; ++i) {
          Real den = prim(IPR,k,je-j+1,i)*2.0E-3/(Globals::Rgas*prim(IT,k,je-j+1,i));
...
#pragma simd
  ...
  for (int k = ks; k <= ke; ++k)
    for (int j = 1; j <= NGHOST; ++j)
      for (int i = is; i <= ie; ++i)
        prim(IT,k,je+j,i) = prim(IPR,k,je+j,i)/prim(IPR,k,je-j+1,i)*prim(IT,k,je-j+1,i);
